...
Strategy.prototype.authenticate = function (req) {
    ...
    try {
      if (self._passReqToCallback) {
        ...
        if (arity == 4) {
          // This variation allows the application to detect the case in which
          // the issuer and subject of the assertion are different, and permit
          // or deny as necessary. Also sends payload so you can handle any
          // custom claims
          self._verify(payload.sub || payload.iss, payload.iss, payload, verified);
        }
        else if (arity == 3) {
          // This variation allows the application to detect the case in which
          // the issuer and subject of the assertion are different, and permit
  ...
  function doKeyStep() {
    function keyed(err, alg, key) {
      if (err) { return self.error(err); }
      if (!alg) { return self.fail(); }
      ...
      var ok = jwt.verify(assertion, alg, key);
